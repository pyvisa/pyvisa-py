...
class SrqInterruptServer(rpc.UDPServer):
    """UDP RPC server that receives VXI-11 DEVICE_INTR_SRQ (proc 30) interrupts."""
    ...
    def loop(self):
        """Run until the session's stop_flag is set."""
        ...
        while not stop_flag.is_set():
            ...
            if reply is not None:
                rpc._sendto(self.sock, reply, host_port)

    def handle_30(self):
        """Handle DEVICE_INTR_SRQ (procedure 30).

        Unpacks the opaque handle, sends an RPC success reply, then
        asynchronously reads the STB and fires the service_request event.
        """
